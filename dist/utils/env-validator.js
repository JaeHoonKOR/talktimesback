...
const jwt_secret_generator_1 = require("./jwt-secret-generator");
/**
 * 필수 환경 변수 목록
 */
...
function validateEnvironmentVariables() {
    ...
    try {
        ...
        logger_1.serverLogger.info('보안 키 자동 검증/생성 완료', {
    ...
    for (const envVar of REQUIRED_ENV_VARS) {
        ...
        if (envVar in SECURITY_REQUIREMENTS) {
            const requirement = SECURITY_REQUIREMENTS[envVar];
    ...
    // 에러가 있는 경우 서버 시작 중단
    if (errors.length > 0) {
        logger_1.serverLogger.error('환경 변수 검증 실패', new Error('필수 환경 변수 누락'), {
            errors,
            warnings
        });
        console.error('\n=== 환경 변수 검증 실패 ===');
    ...
    // 경고만 출력
    if (warnings.length > 0) {
        logger_1.serverLogger.warn('환경 변수 검증 경고', {
            warnings
        });
        console.warn('\n=== 환경 변수 검증 경고 ===');
        warnings.forEach(warning => console.warn(`- ${warning}`));
    }
    logger_1.serverLogger.info('환경 변수 검증 완료', {
