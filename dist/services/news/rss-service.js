...
exports.fetchNewsFromSource = fetchNewsFromSource;
exports.fetchNewsByCategory = fetchNewsByCategory;
exports.fetchAllNews = fetchAllNews;
exports.deduplicateNews = deduplicateNews;
const axios_1 = __importDefault(require("axios"));
const cheerio = __importStar(require("cheerio"));
const rss_parser_1 = __importDefault(require("rss-parser"));
const uuid_1 = require("uuid");
const news_cron_1 = require("./news-cron");
 ...
 */
function fetchNewsFromSource(sourceId) {
    return __awaiter(this, void 0, void 0, function* () {
        const source = (0, rss_sources_1.getRssSourceById)(sourceId);
        if (!source) {
            throw new Error(`소스 ID ${sourceId}를 찾을 수 없습니다.`);
        }
        try {
            console.log(`🔍 ${source.name} 소스에서 뉴스 가져오는 중...`);
            const feed = yield parser.parseURL(source.url);
            console.log(`📋 ${source.name}에서 ${feed.items.length}개의 항목 발견`);
            // 가져온 RSS 항목을 NewsItem 형식으로 변환
            const newsItems = yield Promise.all(feed.items.map((item) => __awaiter(this, void 0, void 0, function* () {
                // 본문에서 이미지 URL 추출 시도
                let imageUrl = yield extractImageFromContent(item);
                ...
                try {
                    ...
                    if (isNaN(publishedAt.getTime())) {
                        ...
                        if (news_cron_1.VERBOSE_LOGGING) {
...
function fetchNewsByCategory(category) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const results = yield Promise.allSettled(allNewsPromises);
...
function extractImageFromContent(item) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (item.enclosure && item.enclosure.url && ((_a = item.enclosure.type) === null || _a === void 0 ? void 0 : _a.startsWith('image/'))) {
            ...
            if (item.link &&
                ...
                try {
                    const response = yield axios_1.default.get(item.link, {
                        timeout: 3000,
                        headers: {
